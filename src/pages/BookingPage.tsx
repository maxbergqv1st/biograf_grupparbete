...
export default function BookingPage() {
  ...
  const [seats] = useState<BookingSeat[]>(
    bookingState?.seats ?? [
      {
        seatId: 32,
        seatLabel: 'D5',
    ...
    return (
      <section className="mt-8 flex justify-center">
        <BiografCard
          ...
          headerClassName="text-red-500 text-center text-xl"
          className="w-[400px] border-none bg-[#1a1a1a] shadow-none"
        >
      ...
      <section className="mt-8 flex justify-center">
        ...
        >
          <div className="space-y-2 p-4 text-center text-white">
            <p className="font-semibold text-[#b69852]">
              {screening.movieName}
            </p>
            <p>
            ...
            <div className="space-y-1 text-sm">
              {seats.map((seat) => (
                <p key={seat.seatId}>
                  {seat.seatLabel} — {translateCategory(seat.category)} —{' '}
                  {seat.finalPrice} kr
                </p>
              ))}
            </div>
            ...
            <p className="text-sm text-gray-400">
              Bekräftelse skickas till {email}
            </p>
            <p className="text-sm text-gray-400">
              Betalning sker i kassan på biografen.
            </p>
            <BiografButton
  ...
  return (
    <>
      {isMobile ? (
        ...
        <section className="mt-8 flex items-start justify-center gap-6">
          {/* Poster for the movie selected for the screening for this booking */}
          <div className="flex flex-col items-center gap-2">
            <div className="h-[270px] w-[180px] overflow-hidden rounded-xl shadow-md">
              {screening.posterUrl ? (
                <AdvancedImage
                  cldImg={cld
                    .image(screening.posterUrl)
                    .resize(fill().width(180).height(270))}
            ...
            <p className="w-[180px] text-center text-lg font-semibold text-[#b69852]">
              {screening.movieName}
            </p>
          </div>
          ...
          <BiografCard
            ...
            className="w-[500px] border-none bg-[#1a1a1a] shadow-none"
          >
            <div className="grid grid-cols-2 gap-8 p-4 text-white">
              {/* Left column */}
              <div className="space-y-4">
                <div>
                  <p className="font-semibold text-[#B69852]">Plats & Tid:</p>
                ...
                <Separator className="my-2" />
                <div>
                  <p className="font-semibold text-[#B69852]">Biljetter:</p>
                  {seats.map((seat) => (
                    <div key={seat.seatId} className="mb-3">
                      <p>
                        Plats {seat.seatLabel} —{' '}
                        {translateCategory(seat.category)} — {seat.finalPrice}{' '}
                        kr
                      </p>
                    </div>
                  ))}
              ...
              <div className="space-y-4">
                <div>
                  <p className="font-semibold text-[#B69852]">Betalmetod:</p>
                ...
                <input
                  ...
                  className="w-full rounded border border-gray-600 bg-transparent p-2 text-sm text-white"
                />
                {error && <p className="text-sm text-red-500">{error}</p>}
                <div className="space-y-2">
                  <BiografButton
                    className="w-full border-none bg-green-600 hover:bg-green-700"
          ...
          {bookingState?.reservationExpiresAt ? (
            <div className="fixed right-4 bottom-4 rounded-xl bg-[#1a1a1a] px-4 py-3 text-sm text-white shadow-lg">
